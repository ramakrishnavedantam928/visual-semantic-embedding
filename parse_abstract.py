...
def get_image_feat(feat_type, image_folder, orig_split, indices, real_split):
    ...
    if 'fc7' in feat_type:
        ...
        try:
            ...
            # number of images to store is images * captions = 5
            images = np.zeros((len(indices) * 5, 3, 224, 224)) # TODO: Low Priority, make general

            for index, item in tqdm(enumerate(indices)):
                abstract_im = demo.load_abstract_image(folder + imname(prefix, item))
                for j in xrange(5):
